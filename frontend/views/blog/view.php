...
<!--            <a class="news-view_content-backLink" href="#">Вернуться</a>-->
            <?= Html::a( Yii::t('blog', 'Повернутися'), ['/blog/'], ['class' => 'cards-view_content-backLink']) ?>
        </div>

        <div class="content__wrap">
            <div class="row">
                <div class="col-md-12">
                    <h1 class="cards-view_content-title"><?= $blog->name ?></h1>
                    <div class="cards-view_content-wrap">
                        <div class="cards-view_content-date"><?= Yii::$app->formatter->asDate($blog->created_at) ?></div>
                ...
                <div class="col-md-12">
                    <div class="cards-view_content-link-flex">
                        <?php if ($last = $blog->lastBlog) { ?>
                            <?= Html::a(Yii::t('blog', 'Попередний блог'), ['/blog/' . $last->slug], ['class' => 'cards-view_content-link-prev cards-view_nav-link']) ?>
                        <?php } ?>

                        <?php if ($next = $blog->nextBlog) { ?>
                            <?= Html::a(Yii::t('blog', 'Наступний блог'), ['/blog/' . $next->slug], ['class' => 'cards-view_content-link-next cards-view_nav-link']) ?>
                        <?php } ?>
                    </div>
                </div>
            </div>
