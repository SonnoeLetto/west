...
<!--            <a class="news-view_content-backLink" href="#">Вернуться</a>-->
            <?= Html::a( Yii::t('news', 'Повернутися'), ['/news/'], ['class' => 'cards-view_content-backLink']) ?>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="cards-view_content-wrap">
                        <div class="cards-view_content-date"><?= Yii::$app->formatter->asDate($news->created_at) ?></div>
                ...
                <div class="col-md-12">
                    <div class="cards-view_content-link-flex">
                        <?php if ($last = $news->lastNews) { ?>
                            <?= Html::a(Yii::t('news', 'Попередня новина'), ['/news/' . $last->slug], ['class' => 'cards-view_content-link-prev cards-view_nav-link']) ?>
                        <?php } ?>

                        <?php if ($next = $news->nextNews) { ?>
                        <?= Html::a(Yii::t('news', 'Наступна новина'), ['/news/' . $next->slug], ['class' => 'cards-view_content-link-next cards-view_nav-link']) ?>
                        <?php } ?>
                    </div>
                </div>
            </div>
