...
<!--            <a class="news-view_content-backLink" href="#">Вернуться</a>-->
            <?= Html::a( Yii::t('promotion', 'Повернутися'), ['/promotion/'], ['class' => 'cards-view_content-backLink']) ?>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="cards-view_content-wrap">
                        <div class="cards-view_content-date"><?= Yii::$app->formatter->asDate($promotion->created_at) ?></div>
                ...
                <div class="col-md-12">
                    <div class="cards-view_content-link-flex">
                        <?php if ($last = $promotion->lastPromotion) { ?>
                            <?= Html::a(Yii::t('promotion', 'Попередний блог'), ['/promotion/' . $last->slug], ['class' => 'cards-view_content-link-prev cards-view_nav-link']) ?>
                        <?php } ?>

                        <?php if ($next = $promotion->nextPromotion) { ?>
                            <?= Html::a(Yii::t('promotion', 'Наступний блог'), ['/promotion/' . $next->slug], ['class' => 'cards-view_content-link-next cards-view_nav-link']) ?>
                        <?php } ?>
                    </div>
                </div>
            </div>
